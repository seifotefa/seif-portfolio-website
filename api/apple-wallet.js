...
export default async function handler(req, res) {
  ...
  if (process.env.APPLE_WALLET_ENABLED !== 'true') {
    return sendError(res, 404, 'Apple Wallet download is disabled');
  }

  const token = typeof req.query?.token === 'string' ? req.query.token : '';
  if (!isAuthorized(token)) {
    return sendError(res, 401, 'A valid private Apple Wallet access code is required');
  }

  const missingEnvironment = requiredEnvironment.filter((name) => !process.env[name]);
  if (missingEnvironment.length > 0) {
    return sendError(res, 500, 'Apple Wallet signing is not configured yet');
  }

  try {
    const iconPath = path.join(process.cwd(), 'api', 'apple-wallet-assets', 'icon.png');
    const icon2xPath = path.join(process.cwd(), 'api', 'apple-wallet-assets', 'icon@2x.png');
    const [icon, icon2x] = await Promise.all([readFile(iconPath), readFile(icon2xPath)]);
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="seif-otefa-business-card.pkpass"');
    ...
    return sendError(res, 500, 'Apple Wallet pass generation failed');
  }
}
